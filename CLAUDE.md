...
### Chakra UI v3 (Not v2!)
- Reference migration docs in `/docs/chakra-v3/` for v2→v3 changes
- Use `colorPalette` prop (not deprecated `colorScheme`)
- Use `gap`/`columnGap`/`rowGap` for spacing (not `spacing` prop)
- All components auto-generate Panda CSS tokens
...
1. **Never edit `styled-system/`** - This directory is auto-generated by Panda CSS
2. **French first** - UI text and messages should be in French by default
3. **Chakra UI v3 only** - v2 components will cause TypeScript errors
