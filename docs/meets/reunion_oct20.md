# SocioBot

s'entrainer a des entretiens reels

=> doivent avoir une idee des personnes avant l'entretien

pourrais etre le premier message du bot

a quoi servira le bot ?
entrainements avant entretiens ou simulations d'entretiens

a partir d'entretens, deja faitt, les etudaitns vont apprendre a faire des entretiens

parametre pedagiogique
quel effet veut on produire
au dela d'une strict interaction avec un bot

l'etudiant apprends a construire une grille d'entretien

naviguer a travers les entretiens fournie par bilel

utiliser un autre agent pour creer un personna plutot qu'un formulaire

l'agent pose des questions
style creer un chatgpt

---

atelier 2 jours avec

- interface existe

https://projet-bilel.onrender.com/

projetentretien
entretien

- formulaire de precision du personnage
- preselection d'une personnalité

demandes

- [bilel] exprimer le resume du profile en language naturel
- ***

- invite aux atelier
- contact dept informatique

upload des entretiens
gestion de la memoire de la conversation
gestion du contexte

iip pii
